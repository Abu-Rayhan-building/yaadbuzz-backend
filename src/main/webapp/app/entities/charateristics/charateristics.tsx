...
import { getEntities, reset } from './charateristics.reducer';
...
export const Charateristics = (props: ICharateristicsProps) => {
  ...
  return (
    <div>
      <h2 id="charateristics-heading" data-cy="CharateristicsHeading">
        <Translate contentKey="yaadbuzzApp.charateristics.home.title">Charateristics</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="yaadbuzzApp.charateristics.home.refreshListLabel">Refresh List</Translate>
          </Button>
              ...
              <tbody>
                {charateristicsList.map((charateristics, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{charateristics.repetation}</td>
                    <td>
                      {charateristics.userPerDepartment ? (
                        <Link to={`user-per-department/${charateristics.userPerDepartment.id}`}>{charateristics.userPerDepartment.id}</Link>
                      ) : (
                        ''
                      )}
                    </td>
                    <td className="text-right">
                      <div className="btn-group flex-btn-group-container">
                        <Button tag={Link} to={`${match.url}/${charateristics.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        ...
                        <Button
                          tag={Link}
                          to={`${match.url}/${charateristics.id}/edit`}
                          color="primary"
                          size="sm"
                        ...
                        <Button
                          tag={Link}
                          to={`${match.url}/${charateristics.id}/delete`}
                          color="danger"
                          size="sm"
...
const mapStateToProps = ({ charateristics }: IRootState) => ({
  ...
  totalItems: charateristics.totalItems,
  links: charateristics.links,
  entity: charateristics.entity,
  updateSuccess: charateristics.updateSuccess,
});
