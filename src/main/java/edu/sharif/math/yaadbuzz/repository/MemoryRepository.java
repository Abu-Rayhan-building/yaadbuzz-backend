...
public interface MemoryRepository extends JpaRepository<Memory, Long>, JpaSpecificationExecutor<Memory> {
    @Query(
        value = "select distinct memory from Memory memory left join fetch memory.tageds",
    ...
    @Query(
        "select memory from Memory memory where memory.department.id = :depid and (memory.isPrivate = false or :currentuserPerDepartmend MEMBER OF memory.tageds )"
    )
    Page<Memory> findAllWithDepartmentId(Long depid, UserPerDepartment currentuserPerDepartmend, Pageable pageable);

    @Query("select memory from Memory memory where memory.department.id = :depid and :userPerDepartmend MEMBER OF memory.tageds")
    Page<Memory> findAllWithCurrentUserTagedIn(Long depid, UserPerDepartment userPerDepartmend, Pageable pageable);

    @Query(
        "select memory from Memory memory where memory.department.id=:depid and :userPerDepartmend MEMBER OF memory.tageds and (memory.isPrivate = false or :currentuserPerDepartmend MEMBER OF memory.tageds)"
    )
    Page<Memory> findAllWithUserTagedIn(
        Long depid,
        UserPerDepartment currentuserPerDepartmend,
        UserPerDepartment userPerDepartmend,
        Pageable pageable
    );
}
