...
@Repository
public interface CharateristicsRepository extends JpaRepository<Charateristics, Long>, JpaSpecificationExecutor<Charateristics> {
    @Query("select charateristics from Charateristics charateristics where charateristics.userPerDepartment.id = :updId")
    Page<Charateristics> findUsersCharactersInDep(@Param("updId") Long updId, Pageable pageable);

    @Query(
        "select charateristics from Charateristics charateristics where charateristics.userPerDepartment.id = :updId and charateristics.title = :title"
    )
