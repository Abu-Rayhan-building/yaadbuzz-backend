...
public class MemoryNotCrudResource {
    ...
    public MemoryNotCrudResource(DepartmentService departmentService,
	    UserService userService, UserRepository userRepository,
	    MailService mailService, MemoryService memoryService) {
    ...
    @GetMapping("/departments/{depid}/memories")
    public ResponseEntity<List<MemoryDTO>> getMemoriesInDep(
	    final Pageable pageable, @PathVariable Long depid) {
	Page<MemoryDTO> page = this.memoryService.findAllInDepartment(depid,
		pageable);
    ...
    @GetMapping("/departments/{id}/memories/me")
    public ResponseEntity<List<MemoryDTO>> getMyMemoriesInDep(
	    @PathVariable Long id, final Pageable pageable) {
	Page<MemoryDTO> page = this.memoryService
		.findAllWithCurrentUserTagedIn(id, pageable);
	final HttpHeaders headers = PaginationUtil
		.generatePaginationHttpHeaders(
			ServletUriComponentsBuilder.fromCurrentRequest(), page);
	return ResponseEntity.ok().headers(headers).body(page.getContent());
    ...
    @GetMapping("/departments/{id}/memories/user/{userPerDepId}")
    public ResponseEntity<List<MemoryDTO>> getMyMemoriesOfUserInDep(
	    @PathVariable Long id, @PathVariable Long userPerDepId,
