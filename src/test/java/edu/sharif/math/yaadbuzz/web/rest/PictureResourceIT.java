...
class PictureResourceIT {
    ...
    private static final byte[] DEFAULT_IMAGE = TestUtil.createByteArray(1,
	    "0");
    private static final byte[] UPDATED_IMAGE = TestUtil.createByteArray(1,
	    "1");
	...
	restPictureMockMvc.perform(get("/api/pictures?sort=id,desc"))
		...
		.andExpect(jsonPath("$.[*].id")
			.value(hasItem(picture.getId().intValue())))
		.andExpect(jsonPath("$.[*].imageContentType")
			.value(hasItem(DEFAULT_IMAGE_CONTENT_TYPE)))
		.andExpect(jsonPath("$.[*].image").value(
			hasItem(Base64Utils.encodeToString(DEFAULT_IMAGE))));
    }
	...
	restPictureMockMvc.perform(get("/api/pictures/{id}", picture.getId()))
		...
		.andExpect(jsonPath("$.imageContentType")
			.value(DEFAULT_IMAGE_CONTENT_TYPE))
	...
	// Update the picture
	Picture updatedPicture = pictureRepository.findById(picture.getId())
		.get();
	// Disconnect from session so that the updates on updatedPicture are not
	// directly saved in db
	em.detach(updatedPicture);
	PictureDTO pictureDTO = pictureMapper.toDto(updatedPicture);

	restPictureMockMvc
		.perform(put("/api/pictures")
			.contentType(MediaType.APPLICATION_JSON)
			.content(TestUtil.convertObjectToJsonBytes(pictureDTO)))
		.andExpect(status().isOk());
    ...
    @Test
    @Transactional
    void updateNonExistingPicture() throws Exception {
	int databaseSizeBeforeUpdate = pictureRepository.findAll().size();
    ...
    @Test
    @Transactional
    void partialUpdatePictureShouldThrown() throws Exception {
	// Update the picture without id should throw
	Picture partialUpdatedPicture = new Picture();
	...
	// Delete the picture
	restPictureMockMvc
		.perform(delete("/api/pictures/{id}", picture.getId())
