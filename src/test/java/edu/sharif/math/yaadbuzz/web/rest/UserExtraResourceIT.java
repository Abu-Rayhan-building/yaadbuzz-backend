...
class UserExtraResourceIT {
	...
	restUserExtraMockMvc
		.perform(post("/api/user-extras")
			.contentType(MediaType.APPLICATION_JSON)
			.content(TestUtil
				.convertObjectToJsonBytes(userExtraDTO)))
	...
	assertThat(testUserExtra.getId())
		.isEqualTo(testUserExtra.getUser().getId());
    }
	...
	// Load the userExtra
	UserExtra updatedUserExtra = userExtraRepository
		.findById(userExtra.getId()).get();
	assertThat(updatedUserExtra).isNotNull();
	// Disconnect from session so that the updates on updatedUserExtra are
	// not directly saved in db
	em.detach(updatedUserExtra);

	// Update the User with new association value
	updatedUserExtra.setUser(user);
	UserExtraDTO updatedUserExtraDTO = userExtraMapper
		.toDto(updatedUserExtra);
	assertThat(updatedUserExtraDTO).isNotNull();

	// Update the entity
	restUserExtraMockMvc
		.perform(put("/api/user-extras")
			.contentType(MediaType.APPLICATION_JSON)
			.content(TestUtil
				.convertObjectToJsonBytes(updatedUserExtraDTO)))
	...
	UserExtra testUserExtra = userExtraList.get(userExtraList.size() - 1);
	// Validate the id for MapsId, the ids must be same
	// Uncomment the following line for assertion. However, please note that
	// there is a known issue and uncommenting will fail the test.
	// Please look at
	// https://github.com/jhipster/generator-jhipster/issues/9100. You can
	// modify this test as necessary.
	...
	// Get all the userExtraList where phone in DEFAULT_PHONE or
	// UPDATED_PHONE
	defaultUserExtraShouldBeFound(
		"phone.in=" + DEFAULT_PHONE + "," + UPDATED_PHONE);
	...
	defaultUserExtraShouldNotBeFound(
		"phone.doesNotContain=" + DEFAULT_PHONE);
    ...
    @Test
    @Transactional
    void getAllUserExtrasByDefaultUserPerDepartmentIsEqualToSomething()
	    throws Exception {
	// Initialize the database
	userExtraRepository.saveAndFlush(userExtra);
	UserPerDepartment defaultUserPerDepartment = UserPerDepartmentResourceIT
		.createEntity(em);
	...
	// Get all the userExtraList where defaultUserPerDepartment equals to
	// defaultUserPerDepartmentId
	defaultUserExtraShouldBeFound("defaultUserPerDepartmentId.equals="
		+ defaultUserPerDepartmentId);

	// Get all the userExtraList where defaultUserPerDepartment equals to
	// defaultUserPerDepartmentId + 1
	defaultUserExtraShouldNotBeFound("defaultUserPerDepartmentId.equals="
		+ (defaultUserPerDepartmentId + 1));
    }
     ...
     */
    private void defaultUserExtraShouldNotBeFound(String filter)
	    throws Exception {
	restUserExtraMockMvc
		.perform(get("/api/user-extras?sort=id,desc&" + filter))
		.andExpect(status().isOk())
	...
	// Get the userExtra
	restUserExtraMockMvc
		.perform(get("/api/user-extras/{id}", Long.MAX_VALUE))
	...
	restUserExtraMockMvc
		.perform(delete("/api/user-extras/{id}", userExtra.getId())
